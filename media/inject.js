...
(function () {
	...
	// Two signals identify the current workspace:
	...
	function activeContext(view) {
		...
		if (titleKey) { keys[titleKey] = true; }

		var rootKeys = {}, rootCount = 0;
		var roots = view.querySelectorAll('.monaco-list-row[aria-level="1"]');
		for (var i = 0; i < roots.length; i++) {
			var k = rowName(roots[i]).toLowerCase();
			if (WORKSPACES[k] && !rootKeys[k]) { keys[k] = true; rootKeys[k] = true; rootCount++; }
		}

		var maps = {};
		var sel = null;
		var sole = null, mapCount = 0;
		for (var key in keys) {
			var map = WORKSPACES[key];
			if (map) { maps[key] = map; sole = key; mapCount++; }
			...
			if (!sel && SELECTIONS[key]) { sel = SELECTIONS[key]; }
		}

		// Where level 1 sits decides what a row's path is relative to. A multi-root
		// window renders the workspace folders themselves as the level-1 rows, so a
		// row's first segment names its root; a single-folder window hides that row
		// entirely and level 1 is already *inside* the folder, whose name is the one
		// the pane header carries. Two roots matching keys settles it on their own;
		// one root only does when the header isn't itself a folder key, which is the
		// single-folder case (a child that happens to share its parent's name).
		var multiRoot = rootCount > 1 || (rootCount === 1 && !WORKSPACES[titleKey]);
		...
		return {
			maps: maps,
			multiRoot: multiRoot,
			// Which map a row belongs to when level 1 is not a root row. The header wins;
			// failing that, a window with exactly one configured workspace can only mean
			// that one. Null leaves `folderAt()` to match across every map.
			soleKey: WORKSPACES[titleKey] ? titleKey : (mapCount === 1 ? sole : null),
